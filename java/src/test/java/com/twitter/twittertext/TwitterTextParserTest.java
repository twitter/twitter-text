...
public class TwitterTextParserTest extends TestCase {
  public void testparseTweetWithoutUrlExtraction() {
    assertEquals("Handle null input", 0, TwitterTextParser.parseTweetWithoutUrlExtraction(null).weightedLength);
    assertEquals("Handle empty input", 0, TwitterTextParser.parseTweetWithoutUrlExtraction("").weightedLength);
    ...
    assertEquals("Count hashtags, @mentions and cashtags normally", 38,
        TwitterTextParser.parseTweetWithoutUrlExtraction("Text with #hashtag, @mention and $CASH").weightedLength);
    ...
    assertEquals("t.co URLs should be counted without transformation", 39,
        TwitterTextParser.parseTweetWithoutUrlExtraction("Text with t.co url: https://t.co/foobar").weightedLength);
  }
}
